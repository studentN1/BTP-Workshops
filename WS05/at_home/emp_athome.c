...
#define SIZE 4			// Number of Employees
	...
	do {
		...
		case 3: // Update Employee Data
			...
			do {
				...
				if (tempEmpID <= 0) {
					printf("ERROR: EmployeeID should be greater than zero !!\n");
				}
			...
			if (check >= 0) {
				...
				scanf("%lf", &emp[check].salary);
				printf("\n");
				check = -1;
			}
			else {

				printf("ERROR!!! Employee does not exist\n\n");
			}
		...
		case 4: // Remove Employee
			...
			if (check >= 0) {
				...
				emp[check].empID = 0; //Employee ID is made zero now to remove the employee
				check = -1;
				empNo--;
